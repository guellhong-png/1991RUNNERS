...
const ElevationChart = ({
    ...
    const points = eles.map((e, i) => `${toX(i).toFixed(1)},${toY(e).toFixed(1)}`).join(' ')
    const areaPoints = `${toX(0).toFixed(1)},${(padT + chartH).toFixed(1)} ${points} ${toX(eles.length - 1).toFixed(1)},${(padT + chartH).toFixed(1)}`

    const yTicks = Array.from({ length: 5 }, (_, i) => {
          const val = minEle + (range * i) / 4
    ...
    return (
          <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                        <p className="text-xs text-gray-400">고도 프로필</p>p>
                        <div className="flex gap-3 text-xs text-gray-500">
                                  <span>최저 {Math.round(minEle)}m</span>span>
                                  <span>최고 {Math.round(maxEle)}m</span>span>
                        </div>div>
                </div>div>
                <svg viewBox={`0 0 ${W} ${H}`} className="w-full rounded-lg bg-gray-50">
                  {yTicks.map(({ val, y }, i) => (
                      <g key={i}>
                                  <line
                  ...
                  {xTicks.map(({ x, label }, i) => (
                      <g key={i}>
                                  <line
                                                  x1={x.toFixed(1)} y1={padT + chartH}
                                                  x2={x.toFixed(1)} y2={padT + chartH + 4}
                                                  stroke="#d1d5db" strokeWidth="1"
                                                />
                                  <text
                                                  x={x.toFixed(1)} y={H - 5}
                                                                ...
                                                                <>
                                                                  ...
                                                                  {routes.length === 0 ? (
                                                                          <div className="card text-center py-16">
                                                                                    ...
                                                                                    <p className="text-gray-400">아직 등록된 코스가 없어요</p>p>
                                                                                    <p className="text-gray-400 text-sm mt-1">첫 번째 GPX를 업로드해보세요!</p>p>
                                                                          </div>div>
                                                                        ) : (
                                                                                                                      ...
                                                                                                                      <div key={route.id} className="card">
                                                                                                                                      <div className="flex items-center gap-3 mb-3">
                                                                                                                                                        ...
                                                                                                                                                        <div className="flex-1 min-w-0">
                                                                                                                                                                            <p className="font-medium text-gray-900 text-sm">{route.author?.name}</p>p>
                                                                                                                                                                            <p className="text-xs text-gray-400">{format(new Date(route.created_at), 'M월 d일 HH:mm', { locale: ko })}</p>p>
                                                                                                                                                          </div>div>
                                                                                                                                      ...
                                                                                                                                      <p className="font-bold text-gray-900 mb-3">{route.title}</p>p>
                                                                                                                        ...
                                                                                                                        {route.elevation_profile && (
                                                                                                                                      ...
                                                                                                                                      <div className="grid grid-cols-3 gap-2 mb-3">
                                                                                                                                        {route.distance != null && (
                                                                                                                                            <div>
                                                                                                                                                                  <p className="text-base font-bold text-gray-900">{route.distance.toFixed(2)} km</p>p>
                                                                                                                                                                  <p className="text-xs text-gray-400">거리</p>p>
                                                                                                                                              </div>div>
                                                                                                                                                        )}
                                                                                                                                        {route.elevation_gain != null && (
                                                                                                                                            <div>
                                                                                                                                                                  <p className="text-base font-bold text-gray-900">↑ {route.elevation_gain} m</p>p>
                                                                                                                                                                  <p className="text-xs text-gray-400">총 상승</p>p>
                                                                                                                                              </div>div>
                                                                                                                                                        )}
                                                                                                                                        {route.elevation_loss != null && (
                                                                                                                                            <div>
                                                                                                                                                                  <p className="text-base font-bold text-gray-900">↓ {route.elevation_loss} m</p>p>
                                                                                                                                                                  <p className="text-xs text-gray-400">총 하강</p>p>
                                                                                                                                              </div>div>
                                                                                                                                                        )}
                                                                                                                                        </div>div>
                                                                                                                      
                                                                                                                        {route.description && (
                                                                                                                                          <p className="text-sm text-gray-500 mb-3">{route.description}</p>p>
                                                                                                                                      )}
                                                                                                                                            ...
                                                                                                                                            {(comments[route.id] ?? []).map((c: any) => (
                                                                                                                                                                  <div key={c.id} className="flex gap-2">
                                                                                                                                                                                          ...
                                                                                                                                                                                          <div className="flex-1 bg-gray-50 rounded-lg px-3 py-2">
                                                                                                                                                                                                                    <p className="text-xs font-medium text-gray-700">{c.author?.name}</p>p>
                                                                                                                                                                                                                    <p className="text-xs text-gray-600">{c.content}</p>p>
                                                                                                                                                                                                                  </div>div>
                                                                                                                                                                    </div>div>
                                                                                                                                                                ))}
