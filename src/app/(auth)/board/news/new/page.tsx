...
export default function NewNewsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await supabase.from('posts').insert({ title: form.title, content: form.content, category: 'news', is_pinned: false, author_id: user?.id })
    router.push('/board/news'); router.refresh()
    setLoading(false)
  }

  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6"><Link href="/board/news" className="text-gray-400 hover:text-gray-600"><ArrowLeft size={20} /></Link><h1 className="text-2xl font-bold text-gray-900">소식 작성</h1></div>
      <div className="card">
