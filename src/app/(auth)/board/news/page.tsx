  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">뛰꼬양 소식</h1><p className="text-gray-500 mt-1">클럽 최신 소식을 확인하세요</p></div>
        {profile?.role === 'admin' && (
          <Link href="/board/news/new"><button className="btn-primary flex items-center gap-2"><Plus size={18} />글쓰기</button></Link>
        )}
      </div>
      <div className="card p-0 overflow-hidden">
        {(!posts || posts.length === 0) ? (
          <div className="text-center py-16 text-gray-400"><p className="text-4xl mb-3">📰</p><p>등록된 소식이 없습니다</p></div>
        ) : (
