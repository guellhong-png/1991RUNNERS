      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <select
            ...
            className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#c0392b] text-gray-700"
          >
          ...
          <div className="flex border border-gray-200 rounded-lg overflow-hidden">
            <button
              ...
              className={`px-3 py-1.5 text-xs transition-colors ${view === 'list' ? 'bg-[#c0392b] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
            >
              목록
            </button>
            <button
              onClick={() => setView('calendar')}
              className={`px-3 py-1.5 text-xs transition-colors ${view === 'calendar' ? 'bg-[#c0392b] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
            >
              캘린더
            </button>
          </div>
      ...
      {loading ? (
              ...
              <div className="flex gap-3 overflow-x-auto pb-1">
                {urgentRaces.map((race, i) => (
                  <div key={i} className="bg-white border border-orange-100 rounded-lg p-3 min-w-[160px] shrink-0">
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700 inline-block mb-2">
                      {getDday(race.reg_end!)}
                    </span>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">
                              ...
                              {race.reg_end && (
                                <span className="ml-2 text-red-400">마감 {race.reg_end}</span>
                              )}
                            </p>
                          </div>
                          <div className="flex flex-col items-end gap-1.5 shrink-0">
                            ...
                            <a
                              ...
                              <button
                                ...
                                disabled={isExpired}
                                className="text-xs py-1.5 px-3 rounded-lg text-center font-medium border whitespace-nowrap transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={() => setCalMonth(new Date(calYear, calMonthNum - 1, 1))}
              ...
              return (
                <div key={day} className={`bg-white min-h-[70px] p-1 ${isToday ? 'bg-red-50' : ''}`}>
                  ...
                  {events.slice(0, 3).map((ev, j) => (
                    <div key={j} className={`text-[9px] px-1 py-0.5 rounded mb-0.5 truncate ${calEventStyle(ev.type)}`}>
                      {ev.name}
                    </div>
                  ))}
          ...
          <div className="flex gap-4 mt-3 flex-wrap">
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <div className="w-3 h-3 rounded bg-green-100 border border-green-300"></div>접수 시작
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <div className="w-3 h-3 rounded bg-red-100 border border-red-300"></div>접수 마감
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <div className="w-3 h-3 rounded bg-blue-100 border border-blue-300"></div>대회 개최일
            </div>
          </div>
        </div>
      )}
